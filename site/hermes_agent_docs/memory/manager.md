...
- [内置记忆 Provider](/hermes_agent_docs/memory/builtin-provider) — 默认的文件存储实现
- [双 Agent 循环](/hermes_agent_docs/agent/dual-loop) — 记忆在循环中的注入位置
- [子 Agent 委托](/hermes_agent_docs/agent/subagent) — `on_delegation` 钩子
