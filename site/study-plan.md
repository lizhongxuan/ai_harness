...
## 相关项目文件:
- [Claude Code 源码](/claude_code_docs/)
- [Vercel AI SDK 源码](/vercel_ai_docs/)
- [Codex 源码](/codex_docs/)
- [Hermes Agent 源码](/hermes_agent_docs/)
...
| # | 模块 | 关键词 | 权重 |
|---|------|--------|------|
| 1 | [状态机 Agent Loop](/modules/agent-loop) | 多阶段 Phase 管道、流式工具并行执行 | ★★★★★ |
| 2 | [多级上下文压缩](/modules/context-compression) | tool-result-budget → snip → micro → collapse → auto 五级管道 | ★★★★★ |
| 3 | [跨会话记忆系统](/modules/memory-system) | 四类型持久化记忆 + sideQuery 语义检索 | ★★★★ |
| 4 | [多级错误恢复](/modules/error-recovery) | prompt-too-long 压缩恢复、max-output-tokens 升级、模型 fallback | ★★★★ |
| 5 | [Token Budget 管理](/modules/token-budget) | 跨压缩边界的预算追踪 | ★★★ |
| 6 | [推测执行](/modules/speculative-execution) | overlay 预执行 + 用户确认 | ★★★ |
| 7 | [沙箱安全](/modules/sandbox-security) | 沙盒 + 执行原理 + 安全 | ★★★ |

---

## 二、学习资源排序

### 第一优先级：[Vercel AI SDK](/vercel_ai_docs/)（直接对口岗位要求）
...
### 第二优先级：[Claude Code](/claude_code_docs/) 源码（生产级 Agent 架构参考）
...
### 第三优先级：[Codex CLI](/codex_docs/) Runtime（补充视角）
...
### 第 1 周：[Vercel AI SDK](/vercel_ai_docs/) 深读 — 建立 Agent Loop 核心认知

#### Day 1-2：AI 辅助建立全局认知

**目标**：用 AI 快速建立 [Vercel AI SDK](/vercel_ai_docs/) 的架构地图
...
| 文件 | 对应岗位模块 | 重点关注 |
|------|-------------|---------|
| `packages/ai/core/generate-text/generate-text.ts` | [状态机 Agent Loop](/modules/agent-loop) | step 循环、tool roundtrip、finishReason 判断 |
...
### 第 2 周：[Claude Code](/claude_code_docs/) 源码分析 — 学习生产级 Agent 架构

#### Day 1-2：整体架构认知

**[Claude Code](/claude_code_docs/) 的本质**：不是一个 CLI 聊天工具，而是一个完整的 Agent 操作系统：
- 任务运行时（task runtime）
...
#### Day 3-4：[上下文压缩](/modules/context-compression)系统深入（对应岗位五级管道）

**[Claude Code](/claude_code_docs/) 的 7 层上下文防御**（从便宜到昂贵）：
...
| 岗位要求 | [Claude Code](/claude_code_docs/) 实现 | 关键设计决策 |
|---------|-----------------|-------------|
...
#### Day 5-6：[记忆系统](/modules/memory-system)深入（对应岗位跨会话记忆）

**[Claude Code](/claude_code_docs/) 的记忆架构：Markdown 文件，不是向量数据库**
...
当空闲时，[Claude Code](/claude_code_docs/) 进入 4 阶段梦境循环：
...
### 第 3 周：[Codex CLI](/codex_docs/) Runtime + 动手实现核心模块

#### Day 1-2：[Codex CLI](/codex_docs/) Runtime 快速通读
...
**重点关注**：
- [沙箱安全](/modules/sandbox-security)执行模型（与岗位"[推测执行](/modules/speculative-execution)"相关）
- [状态机 Agent Loop](/modules/agent-loop) 设计（对比 [Vercel AI SDK](/vercel_ai_docs/) 和 [Claude Code](/claude_code_docs/)）
- 相对简单，代码量不大，适合快速通读

#### Day 3-4：实现[多级上下文压缩](/modules/context-compression)管道
...
#### Day 5-6：实现模型 Fallback 和[错误恢复](/modules/error-recovery)
...
#### Day 7：实现[推测执行](/modules/speculative-execution)
...
**场景 5：[Token Budget](/modules/token-budget) 耗尽**
```
恢复链:
  1. maxBudgetUsd 硬停止
  2. 保存当前会话状态和任务进度
...
- Bun 的 feature() 宏: 编译时特性开关（[Claude Code](/claude_code_docs/) 用它做内部/外部版本区分）
...
**分布式系统重点**：
- 状态机设计模式
- CQRS（命令查询职责分离）— [Claude Code](/claude_code_docs/) 的 Context Collapse 就是这个模式
...
>
> [Vercel AI SDK](/vercel_ai_docs/) 的 streamText 用线性 step 循环，优点是简单可靠，
> 但缺乏状态机的灵活性——没有显式的 Phase 管道，异常状态处理不够细粒度。
>
> [Claude Code](/claude_code_docs/) 是目前最成熟的生产级实现。它的核心是一个单线程主循环（代号 nO），
> 配合异步消息队列（h2A）实现实时转向。[上下文压缩](/modules/context-compression)用了 7 层防御，
> 从便宜到昂贵逐级触发，关键设计是互斥门控和断路器。
> [记忆系统](/modules/memory-system)选择了 Markdown 文件而不是向量数据库，
> 用 Dream Mode 做定期整合——洞察是 LLM 天生擅长读写文本，瓶颈是维护而不是存储。
>
> [Codex CLI](/codex_docs/) 的沙箱预执行模型给了我[推测执行](/modules/speculative-execution)的灵感。
>
> 基于这些，我的设计方案是：
> 1. [状态机 Agent Loop](/modules/agent-loop)，显式 Phase 管道，每个 Phase 有明确的进入/退出/异常条件
> 2. 五级[压缩管道](/modules/context-compression)，借鉴 [Claude Code](/claude_code_docs/) 的分层防御，加入互斥门控和断路器
> 3. Markdown [记忆](/modules/memory-system) + Dream Mode 整合，sideQuery 用嵌入做语义检索但不污染主上下文
> 4. 多级[错误恢复](/modules/error-recovery)链：compress → upgrade-tokens → model-fallback
> 5. 跨压缩边界的 [Token Budget](/modules/token-budget) 追踪，每次压缩后重新计算
> 6. Overlay [推测执行](/modules/speculative-execution)，类似 Git staging area，用户确认后才提交"

---

## 四、三个 Runtime 横向对比（面试核心素材）

### [Agent Loop](/modules/agent-loop) 对比

| 维度 | [Vercel AI SDK](/vercel_ai_docs/) | [Claude Code](/claude_code_docs/) | [Codex CLI](/codex_docs/) |
|------|--------------|-------------|-----------|
...
### [上下文管理](/modules/context-compression)对比
...
### [错误恢复](/modules/error-recovery)对比
...
## 五、[Claude Code](/claude_code_docs/) 隐藏功能（面试加分项/谈资）
...
- Anthropic 员工在公开仓库使用 [Claude Code](/claude_code_docs/) 时自动进入隐身模式
- 禁止在 commit 消息中包含：内部模型代号、未发布版本号、内部仓库名、"Claude Code" 字样
- 没有强制关闭开关——如果系统不确定是否在内部仓库，卧底模式保持开启
- 证明 Anthropic 日常使用 [Claude Code](/claude_code_docs/) 贡献开源项目

### 3. Dream Mode（梦境模式）
- 空闲时进入 4 阶段[记忆](/modules/memory-system)整合循环
...
1. **你读过源码**：能说"我看过 [Claude Code](/claude_code_docs/) 的 query.ts，核心循环代号是 nO"
2. **你做过横向对比**：能说"[Vercel AI SDK](/vercel_ai_docs/) 的 streamText 和 [Claude Code](/claude_code_docs/) 的主循环有三个关键差异"
3. **你有自己的实现**：能展示你写的 mini agent loop、多级压缩器、[错误恢复](/modules/error-recovery)链
...
| "[推测执行](/modules/speculative-execution)怎么保证一致性？" | Overlay 层 + 快照，类似数据库 WAL 或 Git staging area |
| "[Token Budget](/modules/token-budget) 跨压缩边界怎么追踪？" | 每次压缩后重新计算，预算在 system prompt / 历史 / 工具结果间动态分配 |
...
### 源码和仓库
- [Vercel AI SDK](/vercel_ai_docs/): https://github.com/vercel/ai
- [OpenAI Codex CLI](/codex_docs/): https://github.com/openai/codex
- OpenClaude (社区 Fork): https://github.com/Gitlawb/openclaude
- [Claude Code](/claude_code_docs/) 源码镜像: https://github.com/yasasbanukaofficial/claude-code

### 架构分析文章
- Claw Decode 完整分析: https://clawdecode.net/
- [Claude Code](/claude_code_docs/) 内部深入: https://www.janypka.com/claude-code-internals-a-quick-deep-dive/
- [Agent Loop](/modules/agent-loop) 幕后: https://blog.promptlayer.com/claude-code-behind-the-scenes-of-the-master-agent-loop/
- 架构分析: https://docs.bswen.com/blog/2026-03-25-claude-code-architecture/
- [上下文管理](/modules/context-compression) 7 层防御: https://interviewbrowser.com/blog/claude-code-context-management
- 工具/[记忆](/modules/memory-system)/Hooks/MCP: https://www.penligent.ai/hackinglabs/inside-claude-code-the-architecture-behind-tools-memory-hooks-and-mcp/
...
- [Vercel AI SDK](/vercel_ai_docs/): https://sdk.vercel.ai/docs
