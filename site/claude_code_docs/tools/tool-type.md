...
- [权限模式](/claude_code_docs/tools/permission) — `checkPermissions` 的详细展开
- [工具结果落盘](/claude_code_docs/tools/tool-persist) — `maxResultSizeChars` 驱动的落盘机制
- [ReAct 循环工程化](/claude_code_docs/agent/react-loop) — 工具在阶段 5 的执行流程
- [五层上下文防爆体系](/claude_code_docs/context/five-layers) — `isReadOnly` 对微压缩的影响
