...
- [Token 估算](/claude_code_docs/api/token-estimate) — 不同 Provider 的 token 计数方式不同
- [MCP 协议](/claude_code_docs/api/mcp) — MCP 服务器也需要通过 API 客户端通信
- [Prompt Cache 优化](/claude_code_docs/context/prompt-cache) — buildFetch 中的 client-request-id 用于缓存调试
