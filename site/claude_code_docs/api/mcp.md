...
- [多 Provider 接口](/claude_code_docs/api/multi-provider) — MCP 工具调用最终通过 API 客户端执行
- [工具类型系统](/claude_code_docs/tools/tool-type) — MCPTool 是工具类型系统的一种实现
- [Prompt Cache 优化](/claude_code_docs/context/prompt-cache) — Instructions Delta 的核心目的是保护 cache
- [Hook 系统](/claude_code_docs/agent/hook-system) — MCP 工具调用也经过 PreToolUse/PostToolUse hooks
