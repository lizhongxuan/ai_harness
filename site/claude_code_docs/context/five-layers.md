...
触发后执行 9 维结构化摘要（详见 [压缩意图保持](/claude_code_docs/context/compact-intent)），旧消息全部替换为摘要 + 恢复附件。
...
- [工具结果预算](/claude_code_docs/context/tool-budget) — L1 的两级预算详解
- [压缩意图保持](/claude_code_docs/context/compact-intent) — L4 的摘要设计详解
- [Prompt Cache 优化](/claude_code_docs/context/prompt-cache) — L3 的缓存编辑路径
- [工具结果落盘](/claude_code_docs/tools/tool-persist) — L1 的落盘机制详解
