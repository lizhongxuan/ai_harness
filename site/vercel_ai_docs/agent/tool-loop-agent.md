...
- [generateText 循环](/vercel_ai_docs/agent/generate-text-loop) — 底层循环实现
- [streamText 流式循环](/vercel_ai_docs/agent/stream-text-loop) — 流式版本
- [停止条件](/vercel_ai_docs/agent/stop-condition) — isStepCount 等停止逻辑
- [类型安全工具](/vercel_ai_docs/tools/type-safe-tools) — 工具定义
