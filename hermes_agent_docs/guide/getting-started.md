...
1. [双 Agent 循环](/agent/dual-loop) — 理解核心 Agent Loop 双轨设计
2. [并行工具执行](/agent/parallel-tools) — 理解读写分离的并行策略
3. [上下文压缩器](/context/compressor) — 理解 50% 阈值 + 结构化摘要
4. [技能系统](/skills/skill-system) — 理解 SKILL.md 生态
5. [网关架构](/gateway/architecture) — 理解多平台统一接入
6. [工具注册表](/tools/registry) — 理解自注册模式
7. [记忆管理器](/memory/manager) — 理解 Provider 插件架构
8. [RL Agent 循环](/rl/agent-loop) — 理解训练环境集成
