...
5. **工具执行** — 并行/串行分区（详见[并行工具执行](/agent/parallel-tools)）
...
- [并行工具执行](/agent/parallel-tools) — AIAgent 的工具并行策略
- [迭代预算](/agent/iteration-budget) — 循环终止条件
- [RL Agent 循环](/rl/agent-loop) — HermesAgentLoop 在训练中的使用
- [上下文压缩器](/context/compressor) — AIAgent 的压缩触发逻辑
