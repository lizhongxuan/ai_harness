...
- [五层防爆体系](/context/five-layers) — L4 自动压缩的阈值判断依赖精确 token 计数
- [工具结果预算](/context/tool-budget) — 预算检查使用粗略估算
- [多 Provider 接口](/api/multi-provider) — 不同 Provider 的 countTokens 实现不同
