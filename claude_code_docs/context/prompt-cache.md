...
`ContentReplacementState` 的冻结机制（详见 [工具结果预算](/context/tool-budget)）确保每个 tool_result 的内容在整个会话中保持稳定。被替换的结果每轮用 Map 查找重新应用完全相同的字符串，保证字节级一致。
...
- [工具结果预算](/context/tool-budget) — 决策冻结保证 cache 稳定
- [五层防爆体系](/context/five-layers) — 微压缩的 cache_edits 路径
- [Prompt 分区缓存](/build/prompt-section) — 声明式 prompt 段管理
